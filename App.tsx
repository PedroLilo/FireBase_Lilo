...
export default function App() {
    ...
    return (
        <View style={{ flex: 1, padding: 20 }}>
            <Text style={{ fontSize: 22, marginTop: 50, marginBottom: 20 }}>
                CRUD Tarefas ✅
            </Text>
            <TextInput
                ...
                style={{ borderWidth: 1, padding: 5, marginBottom: 10, borderRadius: 5 }}
            />
            <Button title='Adicionar Tarefa'
                onPress={adicionarTarefa}
            />
            <FlatList data={mensagens}
                keyExtractor={(item) => item.id}
                renderItem={({ item }) => (
                    <View style={{ padding: 8, borderEndWidth: 1, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
